...
#--------------------------------------------------------------------------------------------------#
  ...
  def simulate(self) -> None:
    ...
    for i in tqdm(
                ...
                desc=default_logger.GenerateSring("[navsim] ins path generation", Level.Info, Color.Info),
      ...
      while (sim_count < sim_count_max) and (com_complete == 0):
        ...
        if com_type == 1:
          ...
          if np.linalg.norm(self.__att-att_com) < att_converge_threshold and \
            np.linalg.norm(self.__vel_b-vel_com_b) < vel_converge_threshold:
            com_complete = 1
...
#--------------------------------------------------------------------------------------------------#\
  def to_hdf(self, output_dir_path: str):
      ...
      pva_states_df = pd.DataFrame(
          np.block([self.__time[:,None], self.__lla_pos, self.__enu_pos, self.__enu_vel, self.__eul_ang]),
          columns=['time [s]', 'lat [deg]', 'lon [deg]', 'hgt [m]', 
                               'east [m]', 'north [m]', 'up [m]', 
      ...
      ecef_states_df = pd.DataFrame(
        ...
        columns=['time [s]', 'x [m]', 'y [m]', 'z [m]', 'vx [m]', 'vy [m]', 'vz [m]']
      )
      imu_meas_df = pd.DataFrame(
        np.block([self.__time[:,None], self.__spc_frc, self.__ang_vel]),
        columns=['time [s]', 'fx [m/s^2]', 'fy [m/s^2]', 'fz [m/s^2]', 'wx [rad/s]', 'wy [rad/s]', 'wz [rad/s]']
      )
...
#--------------------------------------------------------------------------------------------------#          
  ...
  def __calc_true_sensor_output(self, pos_n: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    rm_eff = Rn + h
    rn_eff = Re + h
    w_en_n = np.array([ve/rn_eff, -vn/rm_eff, -ve*sinphi/cosphi/rn_eff], dtype=np.float64)
    w_ie_n = np.array([GNSS_OMEGA_EARTH*cosphi, 0.0, -GNSS_OMEGA_EARTH*sinphi], dtype=np.float64)

    # calculate rotation rate of body frame w.r.t nav frame
    sinyaw = np.sin(self.__att[0])
    cosyaw = np.cos(self.__att[0])
    w_nb_n = np.array([-sinyaw*self.__att_dot[1] + self.__C_b_n[0,0]*self.__att_dot[2],
